...
namespace QLMucTieu
{
	public class clsTabMucTieu : clsDBInteractionBase
	{
		#region Class Member Declarations
			private SqlDateTime		m_daDateCurent, m_daDateChange;
			private SqlDouble		m_fViDo, m_fKinhDo;
			private SqlInt32		m_iSoLuong, m_iDistance, m_iId;
			private SqlString		m_sKhuVucDuKien, m_sVungBien, m_sKhuVucHoatDong, m_sMoTa, m_sQuocGia, m_sSoHieu, m_sKieuLoai, m_sFullName, m_sToaDo, m_sGroupsMT, m_sDanhHieu;
		#endregion


		public clsTabMucTieu()
		{
			// Nothing for now.
		}
		...
		public override bool Insert()
		{
			...
			scmCmdToExecute.CommandText = "dbo.[pr_tabMucTieu_Insert]";
			...
			try
			{
				...
				scmCmdToExecute.Parameters.Add(new SqlParameter("@sGroupsMT", SqlDbType.NVarChar, 50, ParameterDirection.Input, false, 0, 0, "", DataRowVersion.Proposed, m_sGroupsMT));
			...
			catch(Exception ex)
			{
				...
				throw new Exception("clsTabMucTieu::Insert::Error occured.", ex);
			}
			finally
			{
				// Close connection.
		...
		{
			SqlCommand	scmCmdToExecute = new SqlCommand();
			scmCmdToExecute.CommandText = "dbo.[pr_tabMucTieu_Update]";
			scmCmdToExecute.CommandType = CommandType.StoredProcedure;
			...
			{
				// some error occured. Bubble it to caller and encapsulate Exception object
				throw new Exception("clsTabMucTieu::Update::Error occured.", ex);
			}
			finally
			{
				// Close connection.
		...
		{
			SqlCommand	scmCmdToExecute = new SqlCommand();
			scmCmdToExecute.CommandText = "dbo.[pr_tabMucTieu_Delete]";
			scmCmdToExecute.CommandType = CommandType.StoredProcedure;
			...
			{
				// some error occured. Bubble it to caller and encapsulate Exception object
				throw new Exception("clsTabMucTieu::Delete::Error occured.", ex);
			}
			finally
			{
				// Close connection.
		...
		{
			SqlCommand	scmCmdToExecute = new SqlCommand();
			scmCmdToExecute.CommandText = "dbo.[pr_tabMucTieu_SelectOne]";
			scmCmdToExecute.CommandType = CommandType.StoredProcedure;
			DataTable dtToReturn = new DataTable("tabMucTieu");
			...
			try
			{
				...
				if(dtToReturn.Rows.Count > 0)
				{
					...
					m_sGroupsMT = dtToReturn.Rows[0]["GroupsMT"] == System.DBNull.Value ? SqlString.Null : (string)dtToReturn.Rows[0]["GroupsMT"];
			...
			{
				// some error occured. Bubble it to caller and encapsulate Exception object
				throw new Exception("clsTabMucTieu::SelectOne::Error occured.", ex);
			}
			finally
			{
				// Close connection.
		...
		{
			SqlCommand	scmCmdToExecute = new SqlCommand();
			scmCmdToExecute.CommandText = "dbo.[pr_tabMucTieu_SelectAll]";
			scmCmdToExecute.CommandType = CommandType.StoredProcedure;
			DataTable dtToReturn = new DataTable("tabMucTieu");
			SqlDataAdapter sdaAdapter = new SqlDataAdapter(scmCmdToExecute);
			...
			{
				// some error occured. Bubble it to caller and encapsulate Exception object
				throw new Exception("clsTabMucTieu::SelectAll::Error occured.", ex);
			}
			finally
			{
				// Close connection.
		...
		public SqlString sGroupsMT
		{
			...
			set
			{
				SqlString sGroupsMTTmp = (SqlString)value;
				if(sGroupsMTTmp.IsNull)
				{
					throw new ArgumentOutOfRangeException("sGroupsMT", "sGroupsMT can't be NULL");
				}
				m_sGroupsMT = value;
			}
		}
